...
def read_skeleton_filter(file):
    with open(file, 'r') as f:
        ...
        for t in range(skeleton_sequence['numFrame']):
            ...
            for m in range(frame_info['numBody']):#每帧中body的数量
                body_info = {}
                body_info_key = [
                ...
                body_info['jointInfo'] = []
                for v in range(body_info['numJoint']):#每个关节的信息
                    joint_info_key = [
...
def gendata(data_path, out_path, ignored_sample_path=None, benchmark='xview', part='eval'):
    ...
    for filename in os.listdir(data_path):
        ...
        if issample:
            ...
            sample_label.append(action_class - 1) #标签

    with open('{}/{}_label.pkl'.format(out_path, part), 'wb') as f:#使用pickle.dump(),必须以二进制写入file，‘wb'
        pickle.dump((sample_name, list(sample_label)), f)#把sample_name,list(sample_label)对象写入file
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='./data/nturgb+d_skeletons2')
